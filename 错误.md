...
def extract_and_structure_pdf(pdf_path: str, top_cm: float = 0, bottom_cm: float = 0) -> str:
    # 读取PDF文件
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Title", "Content"])  # 写入标题行
        writer.writerows(structured_data)

    return csv_path

def process_pdf_with_clipping(pdf_path: str, top_cm: float = 0, bottom_cm: float = 0) -> str:
    pdf = fitz.open(pdf_path)
    filename = os.path.splitext(os.path.basename(pdf_path))[0]
    csv_path = f"outputs/{uuid4().hex}_{filename}.csv"
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for page in pdf:
            ...
            for block in text:
                writer.writerow([block[4].strip()])  # 写入内容
